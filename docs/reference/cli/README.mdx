...
### `otterize network-mapper visualize [--format (png/jpg)] [-n <namespace1>,<namespace2>,...] [-o <path>]`
Visualize an access graph for network mapper intents using go-graphviz.

#### Options

| Name | Default | Description |
| --- | --- | --- |
| `--format` | `jpg` | Graph output format (png/jpg) (default "jpg"). |
| `-n` or `--namespaces` |  | Visualize only clients in these namespaces (comma-separated) |
| `-o` or `--output-path` |  | Output file path |

#### Returns

Here's the graph exported after running `otterize network-mapper visualize -n otterize-ecom-demo`:
...
Return the network map built by the network mapper since it started, or since it was reset,
as [client intents file(s)](/reference/intents-and-intents-files/#intents-file-formats) in various formats.
...
### `otterize intents list [--client-service-id <id>] [--server-service-id <id>] [--env-id <id>]`

Returns the list of (applied) intents, optionally filtered by client, server, or environment.

#### Options

| Name | Default | Description |
| --- | --- | --- |
| `--client-service-id` |  | Return only intents from the specified client service. |
| `--server-service-id` |  | Return only intents to the specified server service. |
| `--env-id` |  | Return only intents where either the client or the server belong to the specified environment. |

#### Returns

A table of information about the intents.

- When `type` is `HTTP`, the `object` and `action` columns will show the resource paths and methods, respectively.
- When `type` is `KAFKA`, the `object` and `action` columns will show the topic names and operations, respectively.

```shell
id                client service id server service id type object action
───────────────── ───────────────── ───────────────── ──── ────── ──────
intent_ah8dix4uxv svc_dt7k4ay3bd    svc_kqkallmwsq    HTTP
intent_6rhxnyngnh svc_p7qh7hhtmu    svc_r8fm2oiyvs    HTTP
intent_5uzyy3q7aj svc_gji52kfake    svc_p7qh7hhtmu    HTTP
intent_4ekwmaf2n2 svc_o5bg2me9h5    svc_nqhgkxlbwe    HTTP
intent_2lnzhzn49a svc_lkb2omplkk    svc_lkyww4s6ad    HTTP
intent_tz9e5buzcn svc_nqhgkxlbwe    svc_lkyww4s6ad    HTTP
```

### `otterize intents get <intent-id>`

Returns information about a single (applied) intent, specified by its id.

#### Returns

Returns a table of information about the intent.
