...
[Credentials operator](otterize-oss/components/credentials-operator) to automatically:
...
To deploy the operator, [use the Helm chart](otterize-oss/components/intents-operator/helm-chart).
...
### Kafka mTLS & ACLs
The intents operator automatically creates, updates, and deletes ACLs in Kafka clusters running within your Kubernetes cluster according to the declared intents. It does not modify other ACLs. It works together with SPIRE and the [Otterize Credentials operator](otterize-oss/components/credentials-operator) to easily enable secure access to Kafka from client pods, all in your Kubernetes cluster.
