...
By default, the operator automatically creates a ClientIntent for itself to this server, so that it's able to connect. Default value for this configuration is `false`. To disable, set to `true`.
