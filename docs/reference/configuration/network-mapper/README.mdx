...
- Kafka watcher - the Kafka watcher periodically examines logs of kafka servers provided by the user, parses them and deduces topic-level access to Kafka from pods in the cluster.
- Istio watcher - the Istio watcher queries for all pods with the `security.istio.io/tlsMode` label, queries each pod's Istio sidecar for metrics about connections, and deduces connections with HTTP paths between pods covered by the Istio service mesh.
