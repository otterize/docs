...
To block all traffic within a namespace you can apply a default deny network policy. After applying this policy, only traffic allowed by ClientIntents and other network policies will be allowed.
This policy only blocks ingress traffic, so it does not prevent pods from connecting to the Internet. Enforcement is performed at the ingress to pods. This is appropriate for achieving in-cluster zero-trust and network segmentation.
...
If using Calico for network policies, you can deploy a global, cluster-wide ingress default deny network policy. It is important to exempt Otterize and Calico from this policy so that they are able to function correctly by receiving calls from the Kubernetes API server.
This policy only blocks ingress traffic, so it does not prevent pods from connecting to the Internet. Enforcement is performed at the ingress to pods. This is appropriate for achieving in-cluster zero-trust and network segmentation.
...
## Global egress default deny policy with Calico
If using Calico for network policies, you can deploy a global, cluster-wide default deny network policy. It is important to exempt Otterize and Calico from this policy so that they are able to function correctly.
While Otterize only manages intra-cluster traffic, you may also wish to restrict egress traffic outside of the cluster.

To work well with an ingress default deny policy, an egress policy should not prevent intra-cluster traffic - the ingress policy will already block all in-cluster traffic, and ClientIntents will allow intended traffic.

This example, written by a user on our community Slack, demonstrates how you can achieve this by allowlisting pod IPs.

You will need to adjust the example to allow the IPs for your pods. This is usually part of your CNI or cluster configuration.
...
spec:
  ...
  - action: Allow
    ...
    destination:
      ...
      ports: ["53"]
    # Allow all namespaces to talk to Internet on port 443
  - action: Allow
    protocol: TCP
    destination:
      nets: ["0.0.0.0/0"]
    ...
    # Effectively, ingress default deny would block any intra-cluster traffic
...
After applying a default deny policy, you will need network policies for traffic to go through.
The Otterize network mapper generates ClientIntents for your cluster, but generally speaking you should only use ClientIntents for services that are clients, rather than infrastructure (e.g. checkoutservice is a client, Prometheus is infrastructure).

Many third party Helm charts come with a ready-made network policy to use when deploying with a default deny policy - simply enable them using the values found in the chart.

We recommend using these network policies, as it relieves the infrastructure/platform team in your organization from maintaining ClientIntents for infrastructure services - the Helm chart will always have the most accurate network policies.
