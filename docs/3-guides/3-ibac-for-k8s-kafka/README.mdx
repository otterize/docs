...
- A client that's authenticated but without declared intents:
  - can access `mytopic` successfully;
  - can't access `mytopic-intended`.
- Apply intents for that client to access `mytopic-intended`...
- Now that client can access `mytopic-intended`.
...
one scripted to produce events to `mytopic` and one to consume and show events from `mytopic` &mdash;
...
Sure enough, all that the client (pod) needed in order to access `mytopic` were the mTLS credentials
...
We will now test the behavior with two **new** interactive terminal windows, now scripted to access `mytopic-intended`.
...
Type `hello` into the **producer terminal** and you will see it getting blocked from accessing the `mytopic-intended`:
```bash title="Producer terminal"
>hello
...
Notice that it immediately fails to consume `mytopic-intended` as following:
```bash
...
#### Declare and apply client intents to access `mytopic-intended`

Now we'll declare the client's intents to access `mytopic-intended`:
```yaml
...
Finally, let's again open two interactive terminal windows scripted to access `mytopic-intended`:
...
configured the Kafka ACLs to permit this client to successfully access the `mytopic-intended` topic.
...
### Applying intents
When an intent is applied &mdash; in our case, for `myclient` to access `mytopic` on Kafka &mdash; Otterize automatically
generates an ACL to match the same `CN` value, that is `myclient.ibac-for-kafka`, and to allow it
to access the intended topic with the intended operation.
