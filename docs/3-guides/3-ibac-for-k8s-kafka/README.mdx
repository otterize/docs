...
Security scopes translate directly into Kafka ACLs. You can use the parameters `topic`, `pattern`, `clientIdentityRequired` and `intentsRequired` to configure them. With those, you can choose whether on which topics you'd like to require an identity (enforce mTLS), on which you'd like to require intents, and on which you require none of those. [Read more about Kafka configuration](/components/intents-operator/configuration).

## Otterize identities, intents and Kafka ACLs
In the following example, there is a single client (`myclient`) running in the namespace `ibac-for-kafka`
