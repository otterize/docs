...
To apply intents on an existing cluster you will need to install Otterize with the Kafka/mTLS
option enabled. Follow the [Kafka/mTLS components](/guides/k8s-installation#kafkamtls) installation
instructions to or these [instructions](/guides/k8s-installation) for a complete installation.
:::note
We recommend working with the [Kafka/mTLS components](/guides/k8s-installation#kafkamtls) installation when following this guide to avoid conflicts
with Network Policies applied alongside Kafka ACLs.
:::
...
with permissions for Otterize to configure ACLs.
 ...
 -f https://docs.otterize.com/code-examples/kafka-mtls/helm/values.yaml --create-namespace
 ```
