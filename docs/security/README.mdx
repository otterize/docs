...
The `certificateProvider` could be set to `otterize-cloud` or `cert-manager` to use the other options; see the [Helm chart configuration for the credentials operator](/reference/credentials-operator/helm-chart#cloud-parameters) for more details.
