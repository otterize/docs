...
The cryptographic credentials created by the Otterize Credentials operator (mTLS certificate and key pairs) are provided by a SPIRE server that is deployed alongside the Credentials operator. The Credentials operator watches for pods starting up in a Kubernetes cluster, and if mTLS credentials are requested, it uses the [resolved or declared service name](/service-identities) plus the pod’s namespace and generates credentials for that service name, in that namespace. The operator is conceptually similar to a SPIRE agent in that it attests to the identity of pods.
