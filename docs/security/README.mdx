...
Services in Otterize are identified through their resolved or declared service name (add link to service identity resolution page), coupled with the namespace they’re running in. A service in namespace A has a different identity to a service in namespace B — in network policies as well as cryptographically.
...
The cryptographic credentials created by the Otterize SPIRE integration operator (mTLS certificate and key pairs) are provided by a SPIRE server that is deployed alongside the SPIRE integration operator. The SPIRE integration operator watches for pods starting up in a Kubernetes cluster, and if mTLS credentials are requested, it uses the resolved or declared service name (add link to service identity resolution page) plus the pod’s namespace and generates credentials for that service name, in that namespace. The operator is conceptually similar to a SPIRE agent in that it attests to the identity of pods.
