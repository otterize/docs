...
Consult the guide on [mapping pod-to-pod calls in Kubernetes](/capabilities/k8s-mapping-pod-to-pod-calls) for more.
...
| `allowGetAllResources` | Provides *get*, *list* and *watch* permission to watch on all resources. Used to resolve service names when pods have owners that are custom resources. When disabled, a limited set of permissions is used that only allows access to built-in Kubernetes resources that deploy Pods and Pods themselves - Deployments, StatefulSets, DaemonSets, ReplicaSets and Services. Resolving process may not be able to complete if the owning resource is not one of those. | `true`  |
