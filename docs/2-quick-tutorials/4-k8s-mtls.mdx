...
- Annotate the pod: Otterize looks for the `otterize/credentials-secret-name` annotation, generates
mTLS credentials, and stores them as K8s secrets named according to the annotation value.
- Make the credentials in the K8s secret available as a volume
