...
The network mapper allows you to map network traffic for your K8s cluster.
...
2. Allow you to view pod egress traffic.
3. Avoid using SPIRE (which can be installed with Otterize) for issuing certificates, as Otterize Cloud provides a certificate service.
...
2. Viewing the egress traffic for each pod, including the DNS name and the IPs associated with each outbound request.
...
Note, for example, that the `client` &rightarrow; `server` arrow is yellow. Clicking on it shows the automatically generated Intents for both the client pod to the server pod and the egress of the client to the public internet.
