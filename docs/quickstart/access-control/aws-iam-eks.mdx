...
### Annotate the server pod to create an AWS IAM role
Annotate the server `Pod` so that the Otterize credentials operator creates an AWS IAM role for it and binds its Kubernetes ServiceAccount to the newly created role.
...
```bash
kubectl patch deployment -n otterize-tutorial-iam server -p '{"spec": {"template":{"metadata":{"annotations":{"credentials-operator.otterize.com/create-aws-role":"true"}}}} }'
```
...
But first, let's restart the server. This is necessary so that [AWS IRSA](https://docs.aws.amazon.com/eks/latest/userguide/iam-roles-for-service-accounts.html) will be able to inject the AWS access key and secret as environment variables.

```bash
kubectl rollout restart -n otterize-tutorial-iam deployment/server
```
