...
If you prefer to generate certificates using Otterize Cloud, try [the tutorial for Otterize Cloud](/quick-tutorials/k8s-kafka-mtls).
...
2. Generate certificates using the Otterize Cloud hosted service. If you prefer to generate certificates in-cluster, you can [follow the tutorial for cert-manager](/quick-tutorials/k8s-kafka-mtls-cert-manager).
...
Keep *Use in access graph* **on** under **Kafka ACLs** so that the access graph only shows the authorization status for Kafka ACLs.
...
--set global.issuerName=ca-issuer
...
Let's tell Otterize how to connect to the Kafka broker by applying an Otterize `KafkaServerConfig`, naming it `kafka`.
```bash
...
<br/>Instead of specifying the `credentials-operator.otterize.com/tls-secret-name` pod annotation and mounting a Secret, you can mount the volume from the CSI driver directly.
