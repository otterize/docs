...
Note, for example, that the `client` &rightarrow; `nginx` arrow is dashed. Clicking on it shows:

<div style={{textAlign: 'center'}}>
    <img src="/img/quick-tutorials/network-mapper/otterize-tutorial-access-graph-istio-insights.png" alt="Access graph insights" width="600"/>
</div>
...
3. Showing more specifics about access, if the intents operator is also connected: understand which services are protected or would be protected, and which client calls are being blocked or would be blocked. We'll see more of that in the Istio AuthorizationPolicy tutorial.
...
- Learn how to roll out [Istio authorization-policy-based access control](/quick-tutorials/k8s-istio-authorization-policies) using intents.
- Explore a larger example visually, and [use shadow mode to roll out IBAC](/quick-visual-tutorials/visual-ibac-istio-authorization-policies).
