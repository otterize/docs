...
- Deploy 2 clients calling an nginx reverse-proxy over HTTP using different paths.
- Map their calls using the network mapper and the Istio watcher component.
...
<details>
<summary>Add HTTP request path to Istio exported metrics</summary>
...
The Otterize CLI also supports the `visualize` command that creates an access graph from discovered intents, but it
currently lacks the fine-grained information about HTTP routes, so its output is omitted from the examples above.
...
Note, for example, that the `client` &rightarrow; `server` arrow is dashed. Clicking on it shows:
