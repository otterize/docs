...
when an intent will generate a network policy that can block external traffic. To disable this feature, consult the [documentation for the intents operator](otterize-oss/components/intents-operator).
