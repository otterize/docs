...
For the complete list of annotation parameters please follow the Credentials operator
[documentation](concepts/otterize-oss/componentscredentials-operator/).
...
5. Add pod annotations to let Otterize know it needs to generate the JKS credentials and store them at the specified K8s
   ...
   podAnnotations:
     ...
     credentials-operator.otterize.com/cert-ttl: "31536000" # 1 year
   ```
...
[Read more about Kafka configuration here.](concepts/otterize-oss/componentsintents-operator/configuration).
...
By annotating the pod with `credentials-operator.otterize.com/tls-secret-name: myclient-credentials-secret` Otterize
...
:::tip Join us
Interested in learning more? Let's chat in our [Slack community](https://joinslack.otterize.com/).
:::