...
- **Choose one service to protect** and prepare to protect it. Until then, you'll run in "shadow mode": no network policies will actually be created against this server.
...
Each service is shown as a node in the access graph, while the lines with arrows (edges, in graph-speak) connecting the services show actual access between them, as discovered by the network mapper.
...
- For each **service** you can see its namespace and environment. You can also see its state as a server and as a client, if applicable.
  - As a server, you'll see whether it's protected against unauthorized access and whether it's blocking authorized clients:
    - <span style={{color:"#F3AF3D", fontWeight:"bold"}}>unprotected</span>;
    - <span style={{color:"#238C32", fontWeight:"bold"}}>protected and allowing authorized clients</span>; or
    - <span style={{color:"#E9615C", fontWeight:"bold"}}>protected but blocking authorized clients</span>.
  - As a client, you'll see whether it's allowed to call its servers:
    ...
    - <span style={{color:"#E9615C", fontWeight:"bold"}}>blocked</span> from making some of its calls.
- An **arrow** (&rarr;) indicates an intent by a client to call a server. It's derived from any **discovered intent** &mdash; a call discovered by the network mapper &mdash; and any **declared intent** that the client declared in its `ClientIntents` YAML.
  - Its color indicates the blocking status of calls from that client to that server:
    ...
    - <span style={{color:"#E9615C", fontWeight:"bold"}}>blocked</span>.
...
The access graph shows three services calling the `productcatalogservice`: `frontend`, `recommendationservice`, and `checkoutservice`. The access graph shows an arrow between a client and a server if the network mapper discovered a call attempted ("discovered intent"), or if the client declared an intent to call the server, or both.

Click, for example, on the dashed green arrow from `frontend` &rarr; `productcatalogservice`:
...
- Access would therefore be blocked, once the `productcatalogservice` server was protected.
...
But if you're just getting started with IBAC, and want to first see it in production before getting client devs involved, you can just auto-generate the needed client intents. In fact, you don't need to know in advance which clients call the server: the network mapper will tell you all you need.
:::

Let's ask the network mapper to export all the client intents it discovered that are incoming to the `productcatalogservice`:
...
If we now look at the access graph, lo and behold: the lines and arrows from all 3 clients to `productcatalogservice` are now **solid** green: all the necessary intents have been declared.
...
To do so, recall that we configured Otterize OSS to be in the `defaultShadow` mode: by default, it's in shadow mode for all services, not actually managing network policies for them. To protect a service is a simple matter of applying a `ProtectedService` YAML for it:
...
```shell
kubectl apply -n otterize-ecom-demo -f https://docs.otterize.com/code-examples/shadow-mode/protect-productcatalogservice.yaml
```
...
### How will load balancers, ingress, and other external traffic be handled?
The intents operator automatically detects resources of kind Service (with type LoadBalancer or NodePort), or Ingress, and creates network policies to allow external traffic to relevant pods.

You do not need to configure anything to get this to work. [Learn more.](/reference/configuration/intents-operator#handling-external-traffic)

### Working with admission webhook controllers, such as policy validators like Kyverno
Since you are not placing a global default-deny policy that would affect controllers in your cluster, Otterize will not affect them and they will continue functioning as before.

### Working with Otterize in CI/CD
We recommend placing the ClientIntents and ProtectedService resources alongside the services that own them in your Git repositories:
- ProtectedService alongside the servers, in the Helm chart belonging to the server, or similar
- ClientIntents alongside the clients, in the Helm chart belonging to the client, or similar
...
Lather, rinse repeat, with the access graph providing visibility at each step of the way.
