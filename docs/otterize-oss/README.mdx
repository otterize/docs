...
- The [Otterize intents operator](otterize-oss/components/intents-operator) that translates ClientIntents resources to
...
- The [Otterize credentials operator](otterize-oss/components/credentials-operator) that integrates with SPIFFE/SPIRE
to handle pod identities and manage certificates.
- The [Otterize network mapper](otterize-oss/components/network-mapper) that sniffs pod-to-pod traffic and builds a network map,
