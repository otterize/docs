...
Security scopes are set by setting topics selectors and setting permission levels.
...
Our example is built of a single client (`myclient`) running in the namespace `otterize-tutorial-kafka-mtls-intents`
...
The certificate's `CN` is set to be `myclient.otterize-tutorial-kafka-mtls-intents`. This value will be used in the
...
generates an ACL to match the same `CN` value, that is `myclient.otterize-tutorial-kafka-mtls-intents`, and to allow it
