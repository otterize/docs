...
Our example is built of a single client (`myclient`) running in the namespace `otterize-tutorial-kafka-mtls`
...
The certificate's `CN` is set to be `myclient.otterize-tutorial-kafka-mtls`. This value will be used in the
...
generates an ACL to match the same `CN` value, that is `myclient.otterize-tutorial-kafka-mtls`, and to allow it
