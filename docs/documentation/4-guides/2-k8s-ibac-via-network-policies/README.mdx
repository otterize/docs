...
```
The result should be similar to (if running on EKS):
```
...
(The URL might take some time to populate across DNS servers.)
...
install only the [network policy components](/documentation/guides/k8s-installation#network-policies-only) for Otterize.
...
but all other callers of that server will be blocked. All other *servers* will be untouched -- it's a *gradual* rollout
...
- The `productcatalog` *serves* product information;
...
Here's the intents file of the `checkoutservice` (remember to adjust if you have a different service):
...

Let's apply this intents file, as a developer or a CI/CD process would do:
...

And let's check in on our lab:
...
...
Now let's add the intents file of the `recommendationservice` (remember to adjust if you have a different service):
...

Again, apply it to see what happens when it declares its intents:
...
...
1. Run the network mapper while generating enough traffic in your cluster to capture all calls, and
   ...
   (We've already done that for you if you're using the reference lab: LINK TO EXAMPLE IN GITHUB.)
2. Create a "default-deny-ingress" network policy in your cluster;
...
4. Now apply all the intents you captured in the first step.
   (For the lab, simply run `kubectl apply -f LINK TO AGGREGATED EXAMPLE`.)
...
For more on how to roll out IBAC, see the [guide to implementing IBAC](/documentation/4-guides/6-implementing-ibac.mdx).

SUGGEST OTHER NEXT STEPS AND FINISH BELOW
