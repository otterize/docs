...
<details>
<summary>Expand to see the config: `kafka-mtls-intents/helm/values.yaml`</summary>


```yaml
{@include: ../../../static/code-examples/kafka-mtls-intents/helm/values.yaml}
```
</details>
...
```bash
kubectl apply -f https://docs.otterize.com/code-examples/kafka-mtls-intents/kafkaserverconfig.yaml
```
<details>
...
3. If you check, you should see that an ACL was configured on Kafka
   ...
   ALL from hosts: *, User:CN=myclient.otterize-tutorial-kafka-mtls-intents,O=SPIRE,C=US has ALLOW permission
...
```bash
kubectl delete -f https://docs.otterize.com/code-examples/kafka-mtls-intents/client-intents.yaml
