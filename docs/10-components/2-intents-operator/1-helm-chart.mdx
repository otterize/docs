...
You can use the [operator's Helm chart](https://github.com/otterize/helm-charts/tree/main/intents-operator) to deploy the intents Ooerator on its own, and configure it to work with a SPIRE integration operator to acquire mTLS credentials.
It is recommended to deploy the operator as part of the Otterize chart, which comes with a SPIRE server and the SPIRE integration operator, rather than managing it independently.
