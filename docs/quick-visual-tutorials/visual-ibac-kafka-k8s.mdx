...
:::danger
The deployment of the lab is needed anyways. The previous lab doesn't use mTLS, while this one does. We use a different yaml for the kafka-mtls lab.
:::
<details>
...
:::danger
I updated the instructions to reference the correct yaml.
:::
...
:::danger
Update this picture with the latest UI
:::

Notice that the Kafka service (called `kafka` in this demo) is shown just like any other service that's called by (in this case, 4) other services acting as its clients. We'll let Otterize know this is a Kafka-type service in the next step.
...
:::danger
Verify that the picture, the explanation above, and the intents are all consistent.
:::
