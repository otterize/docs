...
:::danger
Add an image showing only the network mapper connected.
:::
...
:::danger
Add an image of the payment service that's unprotected, would not block, but also has no incoming intents so would not be protected even with enforcement.
