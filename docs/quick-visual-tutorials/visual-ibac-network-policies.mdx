...
In this tutorial, we'll build on the [Kubernetes cluster mapping tutorial](/quick-visual-tutorials/visual-k8s-cluster-mapping) and actually roll out intent-based access control (IBAC),
first in shadow mode without enforcement of access controls, and then turning on enforcement. The access graph will guide us visually in these steps.
...
simulating an e-commerce application, and Otterize installed in the cluster and integrated with Otterize Cloud.
- Use the access graph and shadow mode along with intents to see what would happen once enforcement is turned on.
- Turn on enforcement and verify what happened is what was expected.
...
:::danger
Add an image of the opened `frontend` &rarr; `recommendationservice` edge in this state.
:::
...
:::danger
Add an image of the opened recommendation service in this state.
:::
...
:::danger
Add an image of the opened `frontend` &rarr; `productcatalogservice` edge in this state.
:::
...
:::danger
Add an image of the opened `productcatalogservice` in this state.
:::
...
:::danger
Figure out the easiest way to do this &mdash; do they have to go to the UI, find the integration for this cluster, and substitute it in the Helm chart below? Can they just click on the Connection guide on the cluster itself and alter the Helm command shown there?
:::
...
:::tip What would unintended access look like now?
From now on, if a client attempts a server call that wasn't covered by one of the applied intents, a red line would appear from that client to that server, and the lock on that server would turn red: calls from that client are being blocked.

That may be because:
- The calls didn't happen when the network mapper was building its map from which the intents were bootstrapped, in which case you ay choose to regenerate the intents or create and apply them manually.
