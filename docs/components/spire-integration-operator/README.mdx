...
Alongside the SPIRE integration operator, you could use SPIRE agents and the SPIRE SDK to work with the same SPIRE server. To learn more, check the documentation for [SPIRE](https://spiffe.io/docs/latest/spire-about/spire-concepts/). Note that to use the SPIRE integration operator, you do not need to work directly with SPIRE or SPIRE agents, and can do everything completely using annotations and Kubernetes Secrets.
