...
| `spire-integration.otterize.com/restart-pod-on-certificate-renewal` | A pod with this annotation set to "true" will be restarted after certificate renewal. It will only takes effect if all the pods of an owner has this annotation set. | `false`                                                   |
