...
title: Helm chart
---
You can use the [operator's Helm chart](https://github.com/otterize/helm-charts/tree/main/spire-integration-operator) to deploy the SPIRE integration operator on its own, and configure a SPIRE server for it to work with according to the parameters value.
It is recommended to deploy the operator as part of the Otterize chart, which comes with a SPIRE server and the intents operator, rather than managing it independently.
