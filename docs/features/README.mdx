            ...
            <div role="list" className="tw-grid tw-gap-6 tw-grid-cols-3">
                {features.map((feature) => (
                    <div key={feature.label}
                         className="tw-col-span-1 tw-flex tw-flex-col tw-divide-y tw-divide-gray-200 tw-rounded-2xl tw-bg-white tw-text-center tw-border-1 tw-border-primary-400 tw-border-solid hover:tw-border-primary-600 hover:tw-shadow-md">
                        {feature.items && feature.items[0] && feature.items[0].href &&
                            <a href={feature.items[0].href} className="hover:tw-no-underline hover:tw-font-extrabold">
