...
When enforcement is active &mdash; when it is not disabled &mdash; the intents operator processes applied client intents files and turns each into access authorizations from the client to its called servers: if network policies are active (and supported by the CNI in the Kubernetes cluster), that ensuring there are network policies from that client to all its called servers, and removing any other previous network policies from that client; and similarly for Kafka ACLs to Kafka servers.
...
- For example, you can insist that all services are protected by default, with a global default-deny network policy, and in the case of Kafka with an appropriate restrictive access policy (see the [Otterize KafkaServiceConfig documentation](/reference/configuration/intents-operator/configuration)). All clients must then declare and apply intents in order to reach their intended servers.
...
When enforcement is not active &mdash; when it is disabled &mdash; the intents operator still processes client intents files, but it does not then turn them into access authorizations: no network policies are created or deleted or changed in any way, and similarly no Kafka ACLs are created, deleted, or modified.
...
:::danger
Update the above once the updated UI is available.
:::
